...
// ==========================================
function HomeView({ users, onSaveAttendance }) {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[70vh]">
      <div className="w-full max-w-md bg-slate-800 rounded-3xl shadow-2xl overflow-hidden border border-slate-700 relative">
        <div className="h-32 bg-gradient-to-r from-pink-500/20 to-cyan-400/20 absolute top-0 left-0 w-full"></div>
        <div className="p-8 pt-12 relative z-10 flex flex-col items-center">
          <div className="w-24 h-24 bg-slate-900 rounded-full flex items-center justify-center border-4 border-slate-800 shadow-xl mb-6 overflow-hidden">
            {matchedUser ? (
              <img src={matchedUser.avatar || `https://api.dicebear.com/7.x/avataaars/svg?seed=${matchedUser.name}`} alt="Avatar" className="w-full h-full object-cover" />
            ) : (
              <User size={40} className="text-slate-600" />
            )}
          </div>
          <h2 className="text-2xl font-bold mb-2 text-center">Registro de Asistencia</h2>
          <p className="text-slate-400 text-sm mb-8 text-center">Ingresa tu NIP para buscar tu perfil y confirmar tu asistencia hoy.</p>
          {message && (
            <div className="w-full p-4 mb-6 bg-emerald-500/20 border border-emerald-500/50 text-emerald-300 rounded-xl text-center text-sm">{message}</div>
          )}
          <div className="w-full mb-8">
            <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2">Ingresa tu NIP</label>
            <input 
              type="password" value={nip} onChange={(e) => setNip(e.target.value)} placeholder="••••"
              className="w-full bg-slate-900 border-2 border-slate-700 focus:border-cyan-400 rounded-xl px-4 py-4 text-center text-2xl font-mono tracking-widest text-white outline-none transition-colors"
            />
          </div>
          {matchedUser ? (
...
function AdminLogin({ onSuccess }) {
  ...
  return (
    <div className="w-full max-w-[380px] bg-[#1e293b] p-8 rounded-3xl shadow-2xl border border-slate-700/50">
      ...
      // <p className="text-[10px] text-slate-500 text-center mt-6">Credenciales demo: admin / 1234</p>
    </div>
  );
}
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-slate-800 border border-slate-700 rounded-2xl p-6 flex items-center gap-4">
          <div className="w-12 h-12 bg-cyan-500/20 rounded-full flex items-center justify-center text-cyan-400"><CheckCircle size={24} /></div>
          <div><p className="text-slate-400 text-sm">Asistirán hoy</p><p className="text-2xl font-bold">{todayAttendances.filter(a => a.status === 'Asiste').length}</p></div>
        </div>
        <div className="bg-slate-800 border border-slate-700 rounded-2xl p-6 flex items-center gap-4">
          <div className="w-12 h-12 bg-pink-500/20 rounded-full flex items-center justify-center text-pink-400"><XCircle size={24} /></div>
          <div><p className="text-slate-400 text-sm">No Asistirán</p><p className="text-2xl font-bold">{todayAttendances.filter(a => a.status === 'No asiste').length}</p></div>
        </div>
        <div className="bg-slate-800 border border-slate-700 rounded-2xl p-6 flex items-center gap-4">
          <div className="w-12 h-12 bg-slate-700 rounded-full flex items-center justify-center text-slate-300"><Activity size={24} /></div>
          <div><p className="text-slate-400 text-sm">Sin Confirmar</p><p className="text-2xl font-bold">{users.length - todayAttendances.length}</p></div>
        </div>
      </div>
      <div className="bg-slate-800 border border-slate-700 rounded-2xl overflow-hidden">
        <div className="p-4 border-b border-slate-700 bg-slate-800/50 flex justify-between items-center"><h3 className="font-bold">Estado de los Alumnos</h3></div>
        <div className="divide-y divide-slate-700/50 max-h-[60vh] overflow-y-auto">
          {users.map(user => {
            const att = todayAttendances.find(a => a.userId === user.id);
            let statusBadge = <span className="bg-slate-700 text-slate-300 px-3 py-1 rounded-full text-xs font-medium">Pendiente</span>;
            if (att?.status === 'Asiste') statusBadge = <span className="bg-cyan-500/20 text-cyan-400 border border-cyan-500/30 px-3 py-1 rounded-full text-xs font-medium flex items-center gap-1"><CheckCircle size={12}/> Asiste</span>;
            else if (att?.status === 'No asiste') statusBadge = <span className="bg-pink-500/20 text-pink-400 border border-pink-500/30 px-3 py-1 rounded-full text-xs font-medium flex items-center gap-1"><XCircle size={12}/> No asiste</span>;
            ...
            return (
              <div key={user.id} className="p-4 flex flex-col md:flex-row md:items-center justify-between gap-4 hover:bg-slate-700/20 transition-colors">
                <div className="flex items-center gap-4">
                  <img src={user.avatar || `https://api.dicebear.com/7.x/avataaars/svg?seed=${user.name}`} alt="" className="w-10 h-10 rounded-full bg-slate-900 border border-slate-600" />
                  <div><p className="font-medium">{user.name} {user.lastNameP}</p><p className="text-xs text-slate-400">{user.classTime}</p></div>
                </div>
